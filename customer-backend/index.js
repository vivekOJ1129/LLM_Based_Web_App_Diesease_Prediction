...
app.post('/api/signup', async (req, res) => {
  ...
  try {
    ...
    const verifyUrl = `${process.env.BASE_URL}/verify?token=${token}`; // ✅ Deployed backend link
...
app.get('/verify', async (req, res) => {
  try {
    ...
    const redirectURL = 'https://webpage-euxdl8eq2-vivekoj1129s-projects.vercel.app';

    if (user.verified) {
      return res.send(`<h2>Email already verified. Redirecting...</h2>
    ...
    res.send(`<h2>✅ Email verified! Redirecting to login...</h2>
      <script>setTimeout(() => { window.location.href = '${redirectURL}'; }, 3000);</script>`);
  } catch (err) {
